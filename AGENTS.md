...
- [ ] JEV 接入评估（见 `docs/JEV落地方案-2026-09-20.md`）：Phase 0 用 269 条历史样本（$0.02）测中文精度决定走 API 还是本地复刻；官方明确"英语最佳、CJK 能用但不等同"，要求用户自测
- [ ] 官方 `confidence` 字段警告：第三方逆向得出 `c=(P_max−1/K)/(1−1/K)`，是分布集中度归一化，**非正确性估计**；做阈值分流/校准须用 `probabilities`

---
*最后更新：2026-09-20 - ACH LR 推导下沉（模型只报 code+conf，LR 由代码算）+ JEV 落地方案调研（docs/）*
