...
1. 事实与数字分离：正文出现的任何具体数字（百分比、规模、年份事件）必须来自【材料包】，并在句末标注【据:材料名】；材料包没有的，写【无据待查】，禁止编造。
   **原文优先**：材料包中 `fyp_official_indicators` 是从发改委官网 PDF 原文提取的五年规划指标表（一手），
   `fyp_target_vs_actual`/`fyp_attribution` 是二手汇总。两者冲突时**以官方原文为准**。
    ...
    for sec in judge_scope:
        ...
        try:
            verdict = sub._call_api(
                ...
                "- **带【据:】锚定的数字，一律视为已溯源，不要判为'无出处'**\n"
                "**关键：你的训练知识有截止日期，而本文档写到了 2026 年。**\n"
                "凡是你不知道的近期事件（2024 年后），不得据此判为错误——\n"
                "你的任务是查内部矛盾与明显的历史错误，不是用旧知识否定新事实。\n"
                "只有当你**确信**某断言与长期公认史实冲突时才列出。\n"
