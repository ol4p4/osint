...
def cmd_draft(page, sec):
    ...
    if page.startswith("10"):
        period_hint += """
本节为经济枝叶，必须走**四步对证**结构（主编框架）：
① 该期覆盖的五年规划目标（【据:fyp_target_vs_actual】）
② 对应政策发布节点（【据:policy_timeline】/【据:lgfai_debt_timeline】）
③ 落地结果（达成/超额/落空的实测口径）
④ 成败归因（【判断】性质，显式标注，必须连回该期主导矛盾；【据:fyp_attribution】有归因参照）
另须显式写出"该期规划是否被中断、被什么中断"——分期边界（政治经济阶段）与规划边界（五年行政周期）
的错位处即分析价值最高处。"""
