...
- 反幻觉（2026-09-16 加，9-14 周报曾把 0 条情报硬解读为"空白本身是最强信号"）：
  若上周情报总条数为 0 或极少（<10），第 1 节必须如实写"本周无足够情报数据，以下判断仅基于假设矩阵存量"，禁止把数据缺失解读为任何"信号"；第 4 节预判须明确标注"数据不足，仅供参考"；绝不编造未在材料中出现的事件或数据"""
